...
class HerokuHttpInputTest < Test::Unit::TestCase
  class << self
    def startup
  ...
  def create_driver(conf = CONFIG)
    Fluent::Test::Driver::Input.new(Fluent::Plugin::HerokuHttpInput).configure(conf)
  end
  ...
  def post(messages)
    ...
    req = Net::HTTP::Post.new('/heroku', 'Content-Type' => 'application/logplex-1',
